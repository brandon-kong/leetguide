    ...
    return (
        <div className={cn("py-20 px-5 group  border-2 transition-all border-secondary-500 shadow-sm rounded-xl overflow-hidden aspect-square flex flex-col items-center justify-center hover:border-primary hover:shadow-md bg-primary-50 hover:bg-white", className)}>
            <div className="relative min-h-[100px]">
                ...
                className={'select-none transition-all duration-500 -rotate-[24deg] group-hover:rotate-[24deg]'}
                />

                <Image src={'/icons/triangle-2.svg'} alt={title} width={100} height={100}
                className={'select-none -rotate-12 group-hover:rotate-[12deg] transition-transform duration-1000 absolute inset-0 top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2 '}
                />

                <Image src={'/icons/book-open.svg'} alt={title} width={70} height={70}
                className={'select-none mt-2 duration-700 transition-transform -rotate-12 group-hover:rotate-0 absolute inset-0 top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2 '}
                />
            </div>
